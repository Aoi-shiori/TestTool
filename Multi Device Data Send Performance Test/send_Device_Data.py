...
class CreateDrviceList:
    ...
    def create_Device_List(device_count, SN_Patch):
        device_list = []
        if (len(str(int(device_count))) < 6):
            for item in range(int(device_count)):
                lenth=len(str(int(item)))
                Patch_SN = f"{SN_Patch[0:-lenth]}{str(item)}"
                device_list.append((Patch_SN))
        else:
...
def create_total_patch(ecg_count,ECGPatch,spo2_count,SpO2Patch,bp_count,BPPatch):
    # 拼接sql中需要的values
    ecg_list =CreateDrviceList.create_Device_List(ecg_count, ECGPatch)
    spo2_list = CreateDrviceList.create_Device_List(spo2_count, SpO2Patch)
    bp_list = CreateDrviceList.create_Device_List(bp_count, BPPatch)
    total_list = ecg_list+spo2_list+bp_list
    return total_list
...
def sendDataToVCloud(payload, recordTime, token):
    ...
    try:
        ...
        if json.loads(response.content)['code'] == 200 and json.loads(response.content)[
            ...
            print("{}  ; 设备'{}'：{}--{}数据发送成功".format(time.strftime("%Y-%m-%d %H:%M:%S",time.gmtime()),sensorId,recordTime - 60000,recordTime),sep="\n")
    ...
    if "ECG" in deviceId:
        while True:
            ...
            if record + 60000 < stampEndTime:
                ecgDataList = []
                for i in range(count * 60, count * 60 + 60):

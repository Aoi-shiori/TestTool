...
def assemble_data_ECG(deviceId, recordTime, ecgData, HR,RR,Temp,accStepTotal,timeZone_offset,timezoneName,ProjectId,Subjectid):
    dict = {
...
def single_assembly_data(count, deviceId, record, stampEndTime, stampStartTime,timeZone_offset,timezoneName,ProjectId,Subjectid,data_router):
    ...
    ecgData = assemble_data_ECG(deviceId, recordTime, ecg_list[0],HR=hr,RR=rr,TEMP=temp,accStepTotal=accStepTotal,timeZone_offset=timeZone_offset,timezoneName=timezoneName,ProjectId=ProjectId,Subjectid=Subjectid)
...
def main(startTime, endTime, deviceId,timeZone_offset,timezoneName,ProjectId,Subjectid,DATE_CONFIG,ret_lock):
    ...
    if stampStartTime == stampEndTime:
        ...
        status= single_assembly_data(count, deviceId, record, stampEndTime, stampStartTime,timeZone_offset,timezoneName,ProjectId,Subjectid,data_router)
        if status:
        ...
        status= assembly_data(count, deviceId, record, stampEndTime, stampStartTime,timeZone_offset,timezoneName,ProjectId,Subjectid,data_router)
        if status:
            logger.info(f' {a} 数据组装完毕,准备发送数据... {a}')
            time.sleep(3)
            # 运行异步任务
...
if __name__ == '__main__':
    ...
    for device in Patient_Profile['DeviceId']:
        ...
        for i in range(days, -1, -1):
            ...
            main(P["StartTime"], end_time, device,P["TimeZoneOffset"],P["TimeZoneName"],P["ProjectId"],P["SubjectId"],P["Data_Config"],ret_lock)

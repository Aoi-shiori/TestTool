...
def _get_logger(filename, level='info'):
    ...
    # 输出到文件
    # 日志文件按天进行保存，每天一个日志文件
    file_handler = handlers.TimedRotatingFileHandler(filename=filename, when='D', backupCount=7, encoding='utf-8')
...
filename="XMindToXlsx"
log_filepath='./logs/'
log_filename= os.path.join(log_filepath, f'{filename}.log')

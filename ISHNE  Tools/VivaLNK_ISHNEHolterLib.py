...
class Holter:
    def __init__(self, filename, check_valid=True, annfile=False,read_file=False):
        ...
        if read_file:
            ...
            if check_valid and not self.is_valid():
                ...
                logger.info( "Warning: file appears to be invalid or corrupt. (%s)" % filename )
        # else:
    ...
    def load_header(self):
        ...
        # print(f"放大倍数:{self.ampl_res[0]}")
        logger.info(f"放大倍数:{self.ampl_res[0]}")
        ...
        for i in range(self.nleads):
            self.lead[i] = Lead(lead_spec[i], lead_quality[i], self.ampl_res[i])
...
if __name__ == '__main__':
    # file="./ECGRec_202150_C823468_2022-12-09_ECG_raw.ecg"
    # print(get_datetime(file,0))
    # 替换为您的文件路径
    file = "/Users/rexren/2vivalnk脚本库/Check S3 file/INSHEToJson/api_raw/ECGRec_202222_E111128_1732096402693.ecg"

    # 创建 Holter 实例并加载文件头
    holter = Holter(file, read_file=True)

    # 加载 ECG 数据（振幅）
    holter.load_data()

    # 打印每个导联的振幅数据
    for i, lead in enumerate(holter.lead):
        if lead.data is not None:
            # print(f"导联 {i + 1} 的振幅数据: {lead.data[:10]}")  # 打印前 10 个振幅值
            logger.info(f"导联 {i + 1} 的振幅数据: {lead.data[:10]}") #打印前 10 个振幅值
